...
type IRoleRepository interface {
	...
	GetAllByCondition(query dtos.RoleRequest) ([]model.Role, int, error)
...
func (r *RoleRepository) GetAllByCondition(query dtos.RoleRequest) ([]model.Role, int, error) {
	return r.GetPage(
	`SELECT r.* 
	 FROM role r 
	 WHERE r.role_name LIKE ?
	 AND (? IS NULL OR r.create_time >= ?)
	 AND (? IS NULL OR r.create_time < ?)`,
	query.Page,
	query.Size,
	"%"+query.RoleName+"%",
