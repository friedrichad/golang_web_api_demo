...
type ICustomer interface {
	...
	GetAllByCondition(query dtos.CustomerRequest) ([]model.Customer, int, error)
...
func (r *CustomerRepository) GetAllByCondition(query dtos.CustomerRequest) ([]model.Customer, int, error) {
	return r.GetPage("Select c.* from customer as c "+
	"where (? is Null or c.customer_id = ?))"+
	"and (? is Null or c.customer_name = ?)) "+
	"and (? is null or create_time >= ?) "+
	"and (? is null or create_time < ?) ", query.Page, query.Size, query.CustomerID, query.CustomerName, query.GetDateFrom(), query.GetDateFrom(), query.GetDateTo(), query.GetDateTo())
}
